 ...
 *
 * This version employs 52-bit (~15 microsecond precision) timestamp and 14-bit
 * random-starting (or zero-starting #ifdef ZERO_START_COUNTER) counter.
 *
...
static inline void get_timestamp(uint64_t *out) {
  ...
  *out = (uint64_t)tp.tv_sec << 16;
  *out |= ((uint64_t)tp.tv_nsec << 16) / 1000000000;
}
...
#define RAND_BUFFER_SIZE (1024)
  ...
  if (rand_cursor >= RAND_BUFFER_SIZE - 2 - 7) {
    arc4random_buf(&rand_buf, RAND_BUFFER_SIZE);
    rand_cursor = 0;
  }

#define MAX_COUNTER ((1 << 14) - 1)
#ifdef ZERO_START_COUNTER
#define MAX_COUNTER_START (0)
#else
#define MAX_COUNTER_START ((1 << 14) - 1)
#endif
  static struct State {
    uint_fast64_t timestamp; // 52-bit timestamp
    uint_fast16_t counter;   // 14-bit counter
  } st;
  ...
  out[0] = st.timestamp >> 44;
  out[1] = st.timestamp >> 36;
  out[2] = st.timestamp >> 28;
  out[3] = st.timestamp >> 20;
  out[4] = st.timestamp >> 12;
  out[5] = st.timestamp >> 4;
  out[6] = 0x70 | (st.timestamp & 0xF); // version bits 0b0111
  out[7] = st.counter >> 6;
  out[8] = 0x80 | (st.counter & 0x3F); // variant bits 0b10
  for (int i = 9; i < 16; i++) {
